...
std::string get_xacro(const std::string &xacro_file) {
  std::string xacro_command = "xacro " + xacro_file + " ur_type:=ur10 name:=ur";
...
int main(int argc, char **argv) {
    ...
    std::string urdf_file = ament_index_cpp::get_package_share_directory("ur_description") + "/urdf/ur.urdf.xacro";
    std::string urdf = get_xacro(urdf_file);
    std::string srdf_file = ament_index_cpp::get_package_share_directory("ur_moveit_config") + "/srdf/ur.srdf.xacro";
    ...
    for (auto &pose : samples) {
        do {
            ...
            rs.update();
        } while (scene.isStateColliding(rs, "ur_manipulator", false));
        pose = rs.getGlobalLinkTransform("tool0");
    }
    // write to PCD
