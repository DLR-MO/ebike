...
class UR10(AbstractRobot):
    ...
    def set_config(self):
        xacro_file = get_package_share_directory('ur_description') + '/urdf/ur.urdf.xacro'
        robot_description = get_xacro(xacro_file, {"name": "ur", "ur_type": "ur10"})
        semantic_file = get_package_share_directory('ur_moveit_config') + '/srdf/ur.srdf.xacro'
        robot_description_semantic = get_xacro(semantic_file, {"name": "ur", "ur_type": "ur10"})
